...
namespace laa {
    ...
    // Internal helper
    constexpr auto getNTHeaders(std::vector<std::byte> &data) -> IMAGE_NT_HEADERS32 * {
    ...
    constexpr auto ReadFile(const std::wstring &path, std::vector<std::byte> &outData) -> Result {
	...
	auto set(const std::wstring &path, bool enable, bool backup) -> Result {
		std::vector<std::byte> data;
    ...
    auto resultToString(Result result) -> std::wstring {
        switch (result) {
            case Result::Success:         return L"Success";
            case Result::FileNotFound:    return L"File not found";
            case Result::ReadFailed:      return L"Failed to read file";
            case Result::WriteFailed:     return L"Failed to write file";
            case Result::InvalidPE:       return L"Invalid PE executable";
            case Result::Not32Bit:        return L"Not a 32-bit executable";
            case Result::AlreadyEnabled:  return L"LAA already enabled";
            case Result::AlreadyDisabled: return L"LAA already disabled";
